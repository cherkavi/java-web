...
public final class Context implements Serializable{
	public void debug(String information){
		System.out.println("Context  DEBUG:"+information);
	}
	
	public void error(String information){
		System.out.println("Context  ERROR:"+information);
	}
    ...
    public final boolean GetStatusChange(int timeout, State[] readerStates){
    	debug("GetStatusChange: begin");
    	debug("GetStatusChange: NativeGetStatusChange");
    	int ret = NativeGetStatusChange(timeout, readerStates);
    	debug("GetStatusChange: Check Timeout");    	
    	if (ret == PCSC.E_TIMEOUT)
    		return true;
    	debug("GetStatusChange: Check SUCCESS");
    	if (ret != PCSC.SUCCESS)
    		throw new PCSCException("GetStatusChange()", ret);
    	debug("GetStatusChange: end");
    	return false;
    }
    /**
    ...
    public final Card Connect(String szReader){
    	debug("Connect: begin ("+szReader+")");
    	if (szReader == null) throw new NullPointerException("reader name must not be null");
    	debug("Connect: Get Status:");
    	GetStatusChange(PCSC.INFINITE, null);
    	debug("Connect: Create Card");
    	Card c = new Card();
    	debug("Connect: Native Connect");
