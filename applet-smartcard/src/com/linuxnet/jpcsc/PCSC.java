...
public final class PCSC{
   ...
   /** Tag for requesting card and reader attributes */
    ...
    static{
    		try{
    			System.out.println("PCSC: trying load library");
    			System.gc();
    			// TODO вставить динамическую загрузку библиотеки, в случае, когда апплет "подтягивает" библиотеку с сервера
    			Runtime.getRuntime().load(LoadLibraryFromJar.libraryPath);
    			//Runtime.getRuntime().load("C:/card_driver/jpcsc.dll");
    			System.out.println("PCSC: library LOADED");
    		}catch(Throwable ex){
    			System.out.println("PCSC: ERROR in load library:"+ex.getMessage());
    		}
